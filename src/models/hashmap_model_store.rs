...
pub struct HashmapModelStore<'a, V: ToString + 'a> {
    map: HashMap<&'a str, Box<dyn Model<String, V> + 'a>>,
}

#[async_trait(?Send)]
impl<'a, V: ToString + Clone + From<String> + From<InputType> + Display + 'a>
    ModelStore<'a, String, V> for HashmapModelStore<'a, V>
{
    async fn add(
        &mut self,
        name: &'a str,
...
impl<'a, V: ToString + Clone + From<String> + 'a> HashmapModelStore<'a, V> {
    pub fn new() -> Self {
        Self {
            map: HashMap::new(),
        }
